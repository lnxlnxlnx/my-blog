...
const armasmPath = path.join(__dirname, '..', 'assets', 'vendor', 'armasm.min.js');
if (fs.existsSync(armasmPath)) eval(fs.readFileSync(armasmPath, 'utf8'));
...
const PRE_STYLE = 'background:#fff;border:1px solid #d4d4d4;border-radius:8px;padding:16px 18px;overflow-x:auto;line-height:1.55;font-size:0.88em;font-family:\'SF Mono\',\'JetBrains Mono\',\'Cascadia Code\',Consolas,monospace';

function makePreOpen(lang) {
  return `<pre class="highlight" lang="${lang}" style="${PRE_STYLE}">`;
}

/* 匹配两种格式：<pre class="highlight" lang="x"> 与 <pre><code class="language-x">，幂等可重跑 */
const preBlockRe = /<pre\b([^>]*)>([\s\S]*?)<\/pre>/g;

function processLesson(file) {
  let html = fs.readFileSync(file, 'utf8');
  let count = 0;
  html = html.replace(preBlockRe, (whole, attrs, inner) => {
    let lang = null;
    const langMatch = attrs.match(/\blang="(\w+)"/);
    if (langMatch) lang = langMatch[1];
    const codeLangMatch = inner.match(/<code class="language-(\w+)">/);
    if (codeLangMatch) lang = codeLangMatch[1];
    let text = inner.replace(/<code class="[^"]*">|<\/code>/g, '');
    text = unescapeHtml(stripSpans(text));
    if (!lang) lang = 'plaintext';
    let hl;
    try {
      hl = hljs.highlight(text, { language: lang });
    } catch (e) {
      hl = hljs.highlight(text, { language: 'plaintext' });
    }
    count++;
    return makePreOpen(lang) + inlineStyle(hl.value) + '</pre>';
  });
  if (count === 0) {
